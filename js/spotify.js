import { loading_text, remove_all_children, spotify_call_api, spotify_refresh_access_token } from './functions.js';
//Store for later
let playlist_id = ""
...
function refresh_spotify_playlists(use_dropdown_header, refresh_loading_text) {
    ...
    spotify_call_api("GET", PLAYLISTS).then ((data) => {
        ...
        if(data === 401) {
            ...
            if(use_dropdown_header) add_dropdown_header('playlists');
...
function add_dropdown_header(id) {
    ...
    node.innerHTML = "Select playlist --"
